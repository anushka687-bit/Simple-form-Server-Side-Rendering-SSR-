...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function isFormValid() {
    ...
    return (
      firstName.length >= 1 && surname.length >= 1 &&
      !isNaN(age) && age >= 1 && age <= 100 && gender !== null &&
      collegeName.length >= 1 && course !== '' && branch !== '' &&
      startYear !== '' && endYear !== '' &&
      parseInt(endYear) >= parseInt(startYear) &&
      !isNaN(cgpa) && cgpa >= 0 && cgpa <= 10 &&
      /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) &&
      /^[0-9]{10}$/.test(phone) &&
      city.length >= 1 && /^[0-9]{6}$/.test(pincode) &&
      state.length >= 1 && country !== ''
    );
  }
  ...
  // ── Form submit — POST to MongoDB via backend ──
    ...
    try {
      ...
      if (json.success) {
        ...
        setTimeout(() => {
          ...
          const sname = json.data.firstName + ' ' + json.data.surname;
          window.location.href = `success.html?id=${sid}&name=${encodeURIComponent(sname)}`;
        }, 800);
      } else {
      ...
      showMsg('❌ Cannot connect to server. Make sure backend is running.', false);
      btn.disabled    = false;
      btn.textContent = 'SUBMIT';
    }
  });
